...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        if ($action === 'get_dayhouse') {
            ...
            // Correct query for PostgreSQL (Supabase) using STRING_AGG
            $query = "
                SELECT 
                    sdp.id,
                    sdp.user_id,
                    ...
                    u.Email as email,
                    STRING_AGG(s.name, ', ') as sports_participated
                FROM student_dayhouse_participation sdp
                JOIN users u ON sdp.user_id = u.id
                LEFT JOIN student_dayhouse_sports sds ON sdp.id = sds.participation_id
                LEFT JOIN sports s ON sds.sport_id = s.sports_id
                WHERE sdp.dayhouse_id = :dayhouse_id
                GROUP BY sdp.id, u.First_Name, u.last_Name, u.Email, sdp.joined_at, sdp.fee_paid
